...
// Root `README.md` is Lane A's; `docs/README.md` is not this.
...
const LANES = [
  {
    lane: "A",
    label: "orchestration — Claude Code (`D-227`)",
    test: (p) =>
...
// `D-227` SUPERSEDES `D-200`/`D-201`: LANE A IS ONE OWNER, NOT TWO TOOLS.
//
// This file used to carry a second classifier, `A_SUB`/`subOf`, reporting a
// "Cowork/Code surface crossing" whenever one change spanned the (former)
// Cowork-owned and Code-owned halves of Lane A. `D-227` retired that split:
// Claude Code owns every Lane A surface directly, so there is no second
// surface left to cross. The classifier is REMOVED here, not merely unused —
// a dead classifier that still runs is exactly the kind of instruction that
// outlives the decision that retired it (`B-011`'s defect, repeated).
//
...
export function run() {
  ...
  const { byLane, unmapped } = classify(files);
