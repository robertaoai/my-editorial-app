...
| `journal/2026-08-16-sprint-plan.md` §header | The branch's historical root — `SETUP-SPIKE-000` (`D-185`) | Historical lineage only; grants no current authority. **Current sprint decisions and lane state live in `v1/`; handoff work and its lifecycle live in `handoff/`; enduring feature identity, rank and readiness live in `Modular_PRD.md` §2.5.2**, with version-exclusion conditions in §2.5/§2.5.1. This journal is historical lineage, **not a live backlog** (`B-081` `R2-2`) |
