...
|---|---|---|
| **Request** | Lane B/C feedback: the originating `handoff/` entry. **Judge input:** transient until classified into the relevant existing record under `D-183`. **Customer or business input:** the applicable requirements or refinement tier — `PRD.md` on the demand side, `Modular_PRD.md` on the supply side | Raising a request grants nothing. `D-183` governs Judge approval and clarification **only**; customer input carries no authority at all |
