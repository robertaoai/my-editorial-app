...
| `LINE1_AGENT_ROSTER` | **UNSET** | `OD3` (production only) | Register `D2`. Phase 0 may run a single Line 1 identity for demo purposes; every transition logs `line_assignment` and `agent_id`, so shared executors are visible in data. Roster *shape* is **decided for Phase 0** (`D-163`, `Q1`) — one instance may hold multiple Line 1 roles if each gate records role and executor; only the production roster remains open under `OD3` |
