...
- **Migration `0002`** (additive; `0001` never edited):
  ...
  - `workflow_transitions`: add `line_assignment`, ~~`judgment_independence_status`~~ **— BLOCKED ON Q11 (R1). Do not write this field name until Q11 is confirmed.** Pending confirmation, write `line_boundary_crossed` + `identity_assurance` instead (A21) — **but see `G64` first: `Q11` was deferred past S1 on 2026-08-21 (`D-68`), which selects this fallback by default, and a boolean `line_boundary_crossed` cannot express `FR-05`'s three states or `G-02`'s two false-states without inference at read, which `NFR-03` forbids. Do not write this shape until `G64` resolves** — the old name asserts a cognitive fact the code can only ever prove structurally, and `NFR-02` makes this table append-only, so the wrong name ships permanently. Also add `event_type`, `agent_id`, `agent_run_id`, `supervising_human_id`, `assisting_agent_id`. Extend `actor_type` with `system` — T7 and T10 are System executors and the current enum cannot represent them. **If Q10 answers "product," add a tenancy column in this same migration** (A22) — after S1 it means migrating an append-only table.
