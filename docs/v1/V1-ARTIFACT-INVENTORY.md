...
|---|---|---|
| `scripts/fixtures/` | `D-106`, `G80`, `D-113` `G83`/`G84`, `D-117` **`G91`** | ✅ **Created 2026-08-24.** The negative fixtures, `bun run fixtures`. **The suite and fixture counts are NOT restated here** — this row carried *"six suites, thirty fixtures"* and was stale within a day, which is `G75`/`C-21`'s drift mechanism in the file that lists artifacts. **The runner prints both.** **Fourteen claims of "negative-tested N ways" existed with no fixture in the repository**, and §6.4d told the reader to run one from a session scratchpad. Positive control per suite; asserts the intended **finding**, not merely a failure; `expectDetail` asserts a check's summary line (`G83`); refuses a dirty tree and names what it damaged. **Lane-agnostic since `G91`** — every lane-state mutation used to name a lane letter, so the suite would have broken silently at the first Sprint boundary |
