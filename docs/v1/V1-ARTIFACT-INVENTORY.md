...
| `scripts/checks/lane-state.mjs` | `D-103` | ✅ **Created 2026-08-24.** `C-14` check 15 — **the lane lock as a state machine** (`D-108`, raised as `B-033`): one `Active` with the rest `Blocked`, or none `Active` with the rest `Eligible`. An `Eligible` row beside an `Active` one fails. **The four-state model had never been exercised**: Lane A was `Active` from the moment the table was written, so the invariant held by inertia. The first Sprint-boundary handover is two edits, and a half-applied one — two `Active` rows, or none — was invisible to every other check. Negative-tested six ways |
