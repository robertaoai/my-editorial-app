...
| `docs/v1/V1-B071-CORRECTIVE-PLAN.md` | `B-071` | ✅ **Created 2026-09-02.** Draft implementation plan for `B-071`'s T5/T6 ontology correction, under independent review. **Artifact creation itself is applied** (this row); the *proposal content* (the `D-182` decision, ontology relabel, propagation) remains unapplied until Gate A (`B071-R23`). Supports one correction, not a standing tracker — retire or mark historical, never delete, once `D-182` is recorded and propagated (`B071-R8`). |
...
| `scripts/checks/governed-intent.mjs` | `D-231` | ✅ **Created 2026-09-15.** The shared governed-intent exclusion matcher `graph-coverage.mjs` and `docs-drift.mjs` both import, plus a pure `classifyChangedPaths()` fixtures assert directly. Replaces two independent, previously disagreeing exclusion lists with one — `docs/handoff/**` and `docs/.graphify/**` excluded from both curated-graph coverage and staleness detection. `docs-drift.mjs`'s own `getChangedPaths()` — corrected by `B-109` to call Git via an argument array, never a shell string — lives beside it rather than in this new file, since the shared concern is *what* is excluded, not *how* Git is invoked. **Extended 2026-09-20 (`D-246`):** adds the exact coverage-only manifest (coverage only; `docs-drift` keeps the base matcher), the pure exact-source-path coverage and reverse-reference functions, and their fixtures in `scripts/fixtures/suites.mjs`. `graph-coverage.mjs` is rewritten to use them; `docs/graph-fragments/missing.js` is retained unedited as a reference tool and is no longer the evidence. No file created or retired. |
...
| `lib/config/build-config.ts` | S0 | ✅ **Exists since `43c51ce`, complete.** *This row read ❌ for the whole of `D-94` (`B-006` item 2) — the inventory asserted a file absent that had been committed, and no check compares a row's mark against the filesystem.* **Corrected 2026-08-30 (`D-161`, closing `B-061`)** — the row previously read "incomplete: route constants and canonical flag set not yet implemented," which was itself stale: `EDITORIAL_ROUTE`, `POC_ROUTE` and every `CONFIG_LOG.md` §6–§7 row are implemented, and `config-coupling` is green (50 authoritative rows, 54 declarations) |
| `lib/config/flags.ts` | S0 | ✅ **Exists since `43c51ce` alongside `build-config.ts`, corrected 2026-08-30 (`D-161`, closing `B-061`).** This row previously read "genuinely absent," true when written (`B-002` — a placeholder would have falsely implied undecided flags), but the file was created with S0 and the claim was never updated |
...
| `supabase/migrations/0002_s1_editorial_schema.sql` | **S1 apply — not an S0 artifact** (`D-93`, raised as `B-001`) | ✅ **Written and applied, corrected 2026-08-30 (`D-161`, closing `B-061`).** `G64`/`Q11` no longer gate it — both resolved (`D-112`, `D-111`), `C-30` closed with `0002`'s authorization unconditional (`D-114`), and Lane B wrote and applied it during `LB-S1-02`. `C-33`'s PostgreSQL trigger test passes; local-PostgreSQL unit-tested, live Supabase anon-key behaviour unverified under `DEP-05` |
...
| Local dev stack | **`D-120`**, `C-33`, `DEP-05` | ✅ **provisioned 2026-08-25.** `supabase` CLI as a devDependency (verified `2.115.0`), plus `db:start` / `db:stop` / `db:reset` wrappers so Lane B calls a stable name rather than a binary path, and `.gitignore` entries for `supabase/.temp/` and `supabase/.branches/`. **`supabase/migrations/` and `supabase/config.toml` stay TRACKED** — the migrations are the artifact, the config is the contract, and only the CLI's scratch state is ignored. **The dev database is DISPOSABLE and separate from `main`/production**; `DEP-05` withholds hosted credentials only, which a local stack does not use. **`supabase init` is Lane B's act** — `supabase/` is its surface, so Lane A supplies the tool and does not run it (`D-86`) |
| `docs/specs/ux/` route specification | `G10` → `D-59`, `D-34` — `/editorial` and `/request-brief` | ❌ **Corrected 2026-08-30 (`D-161`, closing `B-061`): the directory does not exist at all** — the prior "exists, empty" claim was wrong. Whether a stack-specific UX spec is required before S2's blind-review/reveal implementation is undecided; not resolved by this correction |
...
| `docs/v1/work-packets/SETUP-SPIKE-000/S2.md` | `D-240` — sprint MMF checklist | ✅ **Created 2026-09-17** |
| `docs/v1/work-packets/SETUP-SPIKE-000/S3.md` | `D-240` — sprint MMF checklist | ✅ **Created 2026-09-17** |
...
**`D-260` (2026-09-24) — no file added or retired, stated explicitly.** The manifest is re-pinned and the
