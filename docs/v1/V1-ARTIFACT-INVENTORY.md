...
|---|---|---|
| `docs/governance/factory-route-operation-crosswalk.md` | `D-173`, `docs/handoff/B-068-*.md` `F4` | ✅ **Created 2026-08-31.** Route master (Sheet 1, all 7 rows `VERIFIED`) and operation master (Sheet 2, 8 rows — 2 `VERIFIED`, 6 `UNVERIFIED`) from both attached CSVs. §3's route-operation join is **deliberately empty** — no CSV column states it and guessing from titles is forbidden by `B-068` §0/§11.5; §4 names the six Chief Editor decisions required to fill it. Reconciles, does not import, the CSVs |
