...
| `G108` | **Open 2026-08-29 — the Next 16 reconciliation surfaced two defects `B-058` did not name, and both break CI rather than typecheck** | **`G108`a — the lockfile no longer matches `package.json`.** `bun.lockb` (2026-08-25, tracked, pins `next@15` among 413 packages under `D-64`) was **not regenerated** when the working tree moved `package.json` to `next@^16.3.3` and `supabase@^2.116.0` (2026-08-29). CI runs **`bun install --frozen-lockfile`** (`SPECS-VERIFICATION-APPARATUS` §4, refined `D-70`) precisely so a resolution cannot drift — **and that command fails when the manifest and lockfile disagree.** `G59` closed on the property that *"the lockfile resolves completely"*; that property is currently false. **`B-058` reported a typecheck failure and this is a different failure at an earlier step**, which is why executing the pipeline finds what reading it does not. **`G108`b — `eslint-config-next` is still pinned `^15.0.0` while `next` is `^16.3.3`.** A major-version skew in the config package that `eslint.config.mjs` extends (`next/core-web-vitals`, `D-66`). `bun run lint` currently passes, so this is **latent, not live** — recorded rather than fixed, because a passing gate is not evidence the pairing is supported. **`G108`c — an untracked `package-lock.json` (2026-08-29, 162 KB) now sits beside `bun.lockb`.** The repo pins `packageManager: bun@1.1.30`; **two lockfiles for one manifest is the ambiguity `G59` was closed to remove**, and npm resolving independently of bun is exactly the non-reproducibility that gap named. **Not fixed here, and deliberately:** all three are `package.json`/lockfile acts, the one thing `docs/handoff/README.md` names as **NOT carved out** while a lane is not `Active` — *"a dependency you need while Lane A is not `Active` genuinely waits."* **The `next.config.ts` repair `B-058` asked for is applied; these three are recorded and wait.** |
