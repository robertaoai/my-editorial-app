...
#### `C-20` — what it must enforce (demoted from `### `, `D-211`)
...
> **Seventh Sprint boundary — 2026-08-30 (`D-159`).** The Chief Editor nominated **Lane B
> `Eligible`**, the selection step `D-156` defines. **Lane A remains `Active`** — a nomination
> does not release the lock; that happens only when the Chief Editor approves it, a separate,
> later act. Not a handover: no condition in §1 changed, and no Lane B task (including the Gap 9
> verification packet drafted this session) is authorized to run by this act alone.
...
| **`LB-S1-03`** | B | Review-only turn, held without the lock (`D-161`, same shape as `LB-S0-02`): the Chief Editor approved Lane B `Active`, Lane B declined the transition — no code execution was ready — and independently verified `B-058` and Phase-2 `B-044` from the Gap 9 packet | `B-060` |
