...
**`G62`a still blocks `R3` DoD D-4 (`D-65`).** Verified 2026-08-21: `bun run typecheck` **exits 2** with ten implicit-`any` errors in `lib/supabase/`, and `bun run lint` **exits 1** — `next lint` is deprecated, **interactive**, and no ESLint config exists. **An interactive prompt in CI hangs rather than fails.** Both must be remediated before *"CI green on the current commit"* is achievable.
