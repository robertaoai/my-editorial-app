...
**Stage A now carries one item: create `eslint.config.mjs`.** `bun run lint` still **exits 1** — `next lint` is deprecated and **interactive**, and **an interactive prompt in CI hangs rather than fails**. *"CI green on the current commit"* is blocked solely on that file.
