...
(`.github/PULL_REQUEST_TEMPLATE/v1-mmf.md`) is created by the same decision and selects nothing. The
...
**Lane A single ownership (`D-227`, superseding `D-200`/`D-201`/`D-202`).** One lane, one lock, one `Active` row, one owner — the phase register still carries **no tool dimension**: **Phase 1 is Lane A**, full stop. **Claude Code owns, updates, validates and commits every canonical governance and documentation source assigned to Lane A** — `docs/`, `scripts/`, `.claude/`, `.agents/`, `.codex/`, `.github/` except `workflows/`, the rule files, and build config. There is no second Lane A tool with its own content surface. **Claude Cowork's role is narrower and lives inside `docs/handoff/`** — it may receive and analyse Lane B/C handoff entries and draft Lane A's answer text there; it does not own, edit or commit any canonical source. `docs/handoff/` stays unmapped (`D-90`); `.github/workflows/` stays Lane C's (`D-75`). Claude Code returns work to Cowork **through the register** — its turn report **is** the boundary decision section, cited from `V1-PHASE-CLOSURE.md` §5.0a (`D-138`), unaffected by `D-227`. **The `A-NNN` series is withdrawn** (`D-203`, against `G102`); `ENTRY_FILE` stays `[BC]` and no reader changes (`D-209`). This section's own obsolete `A-` activation tail was found and removed by `D-213`, which replaced it with this record rather than leaving a channel-that-no-longer-exists instruction live. **`C-35`'s tool-crossing family (Cowork/Code sub-surface crossing) was a live classifier** (`A_SUB`/`subOf`/`bySub` in `scripts/checks/lane-boundary.mjs`) with zero test coverage — untested is not unimplemented (`D-228` corrects `D-227`'s earlier "retired before implementation" claim). The classifier is now **removed outright**: there is no second Lane A surface left to classify. The **lane-crossing family** (`D-88`, Lane A/B/C) is unaffected and remains a separate, live control.

**Sprint freeze state (`D-203`).** A sprint freezes when its successor opens. **`S0` and `S1` are FROZEN** — both completed with artifacts (`build-config.ts`/`flags.ts`; `0002_s1_editorial_schema.sql`) and both were succeeded. **`S2` is the current scope** and is held under `D-171`. **V1 is NOT frozen** — a version freezes only when its successor opens, and `V2` does not exist. A change to a frozen sprint is a backlog item naming its target sprint, never an edit.
...
| Monetization features | **`V1/NG-03`, Charter-level — excludes `PBL-02`** (`D-188`). *Paraphrase corrected `D-188`: this row previously read "Revenue is a business activity, never a product feature (`D-04`)". **`D-04` says no such thing** — it reads "Revenue may attach to the business and the service, never to **a specific editorial decision**", an editorial-independence control that never spoke to product features. `D-04` is unchanged; the paraphrase was the defect.* Revenue is a business outcome; monetization **software** may be a future capability, which is why `PBL-02` has an identity and no rank. `D-96`: the POC commercial flow is original project scope; Stripe scaffolding was removed by `X8` (`D-121`, verified `67706ca`). **Rationale and version-reconsideration condition live in `Modular_PRD.md` §2.5 (`D-148`); capability identity lives in §2.5.2** — cited here, not restated |
| `G113` — automated semantic-attribution check | **Backlog, not scheduled against S0–S5.** `bun run check` verifies that an `OD`/decision citation *arrives* at a location (`tier-sweep`), not that it names the *correct* governing decision — confirmed by negative test when `D-168` restored `AC-17`'s retired `OD3`-blocked wording and the full suite stayed green. `scripts/checks/` is Lane A's own surface (`D-84`), so this is this lane's future work, not a cross-lane dependency. No phase gates on it |
...
| **`T1`–`T5` phase-gate/task-execution model — spec propagated, construction not authorized** (`D-234`/`D-235`/`D-239`) | The role-selection lifecycle, `T5` RACI normalization, Final Sign-Off `A`-coverage extension, Chief Journalist child-task draft (dissolved, `D-236`) and role-identity split (`D-238`) are documented at `requirements-traceability-map.md` §6.3, `factory-route-operation-crosswalk.md` §4.2, `FN-GATES-01-05.md` §3.0a/§4.2 and the storyboard overlay (Panel A9). **`D-239` reframes V1's actual boundary as UI/UX assistance for newsworthiness ranking and routing** (§6.4) — not workflow execution; `OP-FINAL-SIGNOFF` enforcement, Draft-signature validation, external-GRC exchange, `T6`, and any Line 3 workflow are explicitly outside V1. **No schema, code, persisted field, or API change is authorized by any of this propagation** — it remains a Register/spec-tier act only (`D-54`). **Ready when:** a distinct Chief Editor work order authorizes construction, naming the sprint it lands in |
...
11. **The governed-intent graph excludes the handoff worklog (`D-231`).** `graph-coverage` and `docs-drift` share one matcher (`scripts/checks/governed-intent.mjs`): `docs/handoff/**` and `docs/.graphify/**` are excluded from both curated-graph coverage and staleness detection. A handoff-only commit passes both checks without a rebuild; a governed-doc or code commit still fails until the next `graphify hook-rebuild`; a mixed commit fails — the exclusion is per-path, not per-commit. DoD: `graph-coverage` reports the excluded class, not a bare count; `docs-drift` inspects actual changed paths via an argument-safe Git call (`B-109`) and fails closed if the analyzed commit cannot be resolved; eight fixture cases (five exclusion, three argument-safety) pass from a clean tree. **Amended by `D-246` (2026-09-20):** `graph-coverage` additionally excludes an exact, glob-free **coverage-only manifest** (non-authoritative plan-pack documents and named historical journals) that `docs-drift` does **not** exclude; coverage evidence is a node whose `source_file` equals the document's path, not a basename appearing anywhere in the graph; and a manifest journal that a live source later cites fails the check and names the citation. The manifest's identity lives in the Register and `scripts/checks/governed-intent.mjs`, not here. The `D-246` fixture cases pass from a clean tree.
