...
| `DOR-R4` — required Route-1 operation contracts | **`OP-DRAFT` resolved 2026-09-17** — Reporter is Route-1's executor (corrected: an earlier pass here wrongly named Journalist, who belongs to `ROUTE-PROD-2`); Chief Editorial Desk is the recorded application-default `A` (crosswalk §4.1). **`OP-FINAL-SIGNOFF` is ACCEPTED, attesting party corrected 2026-09-17 (`D-237`)** — Chief Editor (Editor-in-Chief) A-only attestation, Desk Editor is `C` not `A`, no synthetic `R`, idempotent replay per revision, fresh attestation on relevant revision, opens human review only (crosswalk §4.1/§4.2). `OP-COPY-EDIT`'s source `R` corrected to Chief Journalist (`D-236`), already `VERIFIED`, unchanged shape | Every required operation has one executable atomic/milestone shape before T5 readiness |
...
| **`T1`–`T5` phase-gate/task-execution model — spec propagated, construction not authorized** (`D-234`/`D-235`) | The role-selection lifecycle, `T5` RACI normalization, Final Sign-Off `A`-coverage extension and Chief Journalist child-task draft are now documented at `requirements-traceability-map.md` §6.3, `factory-route-operation-crosswalk.md` §4.2, `FN-GATES-01-05.md` §3.0a and the storyboard overlay (Panel A9). **No schema, code, persisted field, or API change is authorized by that propagation** — this remains a Register/spec-tier act only (`D-54`). **Ready when:** a distinct Chief Editor work order authorizes construction, naming the sprint it lands in |
