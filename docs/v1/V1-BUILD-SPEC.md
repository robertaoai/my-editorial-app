...
**Lane A is two tools with split surfaces (`D-200`, corrected by `D-201`).** One lane, one lock, one `Active` row — the tools serialize on Lane A's turn, and the phase register carries **no tool dimension**: **Phase 1 is Lane A, both tools** (`D-202`). **Strategy is Claude Cowork** — `docs/` except `docs/handoff/` and `docs/graph-fragments/*.js`, the rule files, `.github/` except `workflows/`, and `docs/graph-fragments/*.json` as curated meaning. **Implementation is Claude Code** — `scripts/`, `.githooks/`, `.claude/`, `.agents/`, `.codex/`, `docs/graph-fragments/*.js`, build and repo config (`package.json`, `tsconfig.json`, `eslint.config.mjs`, `next.config.ts`, `postcss.config.mjs`, `.gitattributes`, `.gitignore`, lockfiles) and root `README.md`. **Two exact-file exceptions apply BEFORE the directory rule**: `.agents/rules/graphify.md` is a rule file and Cowork's; `docs/graph-fragments/*.json` is curated meaning and Cowork's. **Operating a tool is not editing it** — Claude Code runs the Graphify pipeline; `.graphify/` is gitignored runtime metadata owned by no surface. `docs/handoff/` stays unmapped (`D-90`); `.github/workflows/` stays Lane C's (`D-75`). Claude Code returns work to Cowork **through the register** — its turn report **is** the boundary decision section, cited from `V1-PHASE-CLOSURE.md` §5.0a (`D-138`). **The `A-NNN` series is withdrawn** (`D-203`, against `G102`); `ENTRY_FILE` stays `[BC]` and no reader changes (`D-209`). **No `A-` entry is filed until readers, channel documentation, run registration and fixtures agree** (`D-201`); the channel widening and the tool-crossing gate are **two separate controls, both specified, neither applied**.
