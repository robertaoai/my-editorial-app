...
**Stages A, B and C are installed — 2026-08-21 (`D-70`), executing the sequence `D-65` set out.** `eslint.config.mjs` created and the `lint` script repointed to `eslint .` (21 files, 0 findings); `R3` installed — `__tests__/smoke.test.ts`, `bun test`, and `.github/workflows/ci.yml` running typecheck, lint and test as separate steps; `C-14`'s four detection checks installed at `scripts/check-consistency.mjs` and wired into CI. **CI is green on a real run.** Stage D remains blocked — see below.
