...
| **03 · Newsworthiness vs. trend score vs. editorial priority** | `docs/source/blueprint.md` §11.3; `docs/governance/media-industry-sop-fallback-implementation-plan.md` §6.1; `docs/Modular_PRD.md` (`NG-10`, `CR-06`/`FB-06` disposition, §2.5.1) | `D-148` | `f7b3aea` — **flagged 2026-09-25 (`D-265`, `B-137` R4):** this entry says `CR-06` is "not computable on the shipped schema today (`TC3`)". Five scoring inputs now exist in `0002`; `engagement_metrics` is still absent and no scoring engine exists, so the conclusion holds but the reason is narrower. Not yet compared against the hosted text, not republished |
...
|---|---|---|
| **01** | `D-251`: the natural-person access role is now `ACCESS-ROLE-CHIEF-EDITOR`; `ROLE-*` is reserved for virtual-agent identities. If the entry names "Chief Editor" as a Line 2/`T5`/`T6` actor, check which identity it means | ~~Deferred~~ **Discharged 2026-09-24** — hosted comparison and v15 republication (`D-256`); `DOR-R6` confirmed by Lane B's direct read of the v15 export under Option 2 (`D-259`, `docs/handoff/B-131`) |
| **04** | `D-243`/`D-250`: `ManualReady` is a publication event, never an article state; `Published` promotion is a V2 target | **Deferred** — packet `V1-SM06` (Entry 04 DoR box); return condition: a hosted read and a recorded comparison, or the Judge's explicit acceptance naming this entry; owner Lane A with the Judge |
| **05** | `D-247`/`D-249`: V1 satisfies `CR-19` only partially; `V1-SM05` claims no `transition:T*` execution | ~~Deferred~~ **Discharged 2026-09-24** — as Entry 01 (`D-256`, `D-259`) |
| **06** | `D-249`: `V1-SM05` is a limited evidence slice; its review set includes this entry | ~~Deferred~~ **Discharged 2026-09-24** — as Entry 01 (`D-256`, `D-259`) |
