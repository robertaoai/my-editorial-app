...
> **Document type:** Build Readiness Addendum to the Product Blueprint (v1.2), aligned to Project Charter v1 and Business Case v2.1
> **Document version:** v1.4 — Three Lines Model alignment
> **Date:** 2026-08-16
...
- Zero articles bypass review **independence** (four-eyes at Line boundaries: unconditional — satisfied by Line separation per Three Lines Model; ratification pending)
...
| OD3 | Agent headcount and distribution | Resolved (Three Lines Model) | **Yes** — Line 1 (4 roles, combinable), Line 2 (1, human-primary), Line 3 (conditional) | No | — | — | Permission matrix and four-eyes logic use Line assignment, not agent-instance count. |
...
The assumed stack (A2) is: React/Next.js frontend, FastAPI (Python) backend, PostgreSQL database. Supabase Auth handles authentication only. A **mock publish mode** is available for testing without external dependencies.
...
*This addendum (v1.4) is the build specification layer above the blueprint's invariant core, aligned to the Project Charter v1 and Business Case v2.1's Three Lines Model resolution. OD1–OD3 are structurally resolved but remain unratified. All Assumed defaults may proceed as provisional assumptions for prototype development. This document is a self-authored specification, not an independently audited artifact — claims should be verified against source material before being cited as confirmed. Citations increase checkability, not correctness.*
