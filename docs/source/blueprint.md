...
| Four-eyes principle fails at Line boundaries | Low | High | Line separation (`SEC-01`) is the project's own RACI-sourced rule, not a Three Lines Model requirement. Whether it produces genuinely distinct judgment is `OD2` — open, not assumed. Monitor post-launch: if agents in different Lines produce identical judgments due to shared training data, revisit OD4 (pre-launch blocker). See §18 Lexicon Audit and §16 Line 3 Trigger Rules. |
