...
| `SEC-06` | Organizational assurance: Line 3 is independent of Lines 1 and 2 — the entity with the problem is not the entity attesting it is solved | Three Lines Model (IIA, 2020, updated 2024 — re-anchoring to the 2026 Statement of Position tracked separately) | — | Line 3 — **absent for v1** (`X8`/`AS-03`) | **Specified, not built.** Governs system-level assurance, not per-article review; `SEC-01` covers the latter and no longer cites this standard (`D-163`) |
...
> **`SEC-01` and `SEC-06` were one row until `D-163` (closing `Q12`), and they govern different things.** `SEC-01` is the daily editorial four-eyes control — the Responsible party never equals the Accountable party at a Line boundary — and comes from the project's own RACI design, not an external standard. `SEC-06` is the organizational question of whether the whole editorial system is independently assured, which the Three Lines Model actually addresses. Citing the IIA for `SEC-01` was importing a governance-layer standard to source a workflow-layer rule; the rule held regardless of which 2026 Statement of Position superseded which 2020 one, because it was never really about the IIA.
...
| **M2 — Independence** (S2) | FR-04, FR-05; NFR-03, SEC-01 | M1; Q1 | AC-05…AC-08 pass. **Phase 0 done only — production done needs OD1 and OD2 ratified** | Not started — blocked on `Q12` (Line-exclusivity citation) and `Q1`, both open Chief Editor decisions |
...
| S2 | Line assignment and independence | O-03 ⚠ | FR-04, FR-05 | NFR-03, SEC-01 | AC-05…AC-08 | P0 | Not started — blocked on `Q12`, `Q1` |
