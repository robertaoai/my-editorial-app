...
- **Resolution:** Applied
- **Examined-By:** Lane B — **REJECTED**, see `B-023`: its evidence is anchored at a commit whose check has since been rewritten. It stays `Applied` because no `Rejected` resolution exists yet (`D-105` P2, drafted not built); **the rejection is the linked entry, not this field** (`D-108`)
