...
- **Resolution:** Applied
- **Examined-By:** Lane B — **REJECTED**, see `B-024`: the `config-coupling` negative-test evidence it cites was never tracked. It stays `Applied` because no `Rejected` resolution exists yet (`D-105` P2, drafted not built); **the rejection is the linked entry, not this field** (`D-108`)
- **Verified-By:** — not independently verified; Lane A answered and Lane A recorded it (`B-013` item 4)
- **Evidence:** `scripts/checks/config-coupling.mjs` installed as check 12, negative-tested in both directions; found 13 unimplemented rows and 2 `CONFIG_LOG.md` defects on its first run
