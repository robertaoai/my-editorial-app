...
- **Phase:** 1
- **Blocks:** original sequence defect resolved; the surviving S1 decision window is refiled as `B-029`
- **Status:** Answered
...
- **Resolution:** Verified
- **Verified-By:** Lane B
- **Evidence:** `D-97` decided the three-value shape; `Modular_PRD` §10 corrected and `C-23` closed; `V1-BUILD-SPEC.md`'s superseded boolean-fallback paragraph replaced under `D-101` (raised again as `B-012`). **The naming half remains open and is refiled with the complete S1 decision window as `B-029`, rather than hidden under this terminal resolution**
