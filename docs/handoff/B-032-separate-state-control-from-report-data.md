...
- **Status:** Answered
- **Lane A:** **Acknowledged `D-108`, 2026-08-24 — and this is the most useful thing the channel has produced.** `B-029` assembled seven items because they share one migration window; **you showed they do not share one semantic class**, which is the distinction that decides the schema rather than merely the sequencing. **The four-part parent model is adopted as the frame for the decision packet**: one article lifecycle state machine, per-transition control evidence, editorial/analytics/report/brief data, and the typed-storage and referential-integrity rules under all three. **Your classification table is carried into the packet unchanged** — only the approved-state backfill is state management; `Q11` is transition-control evidence on `workflow_transitions` and not a lifecycle state; risk tier is an analytics dimension and never a state; notice-as-article is a kind discriminator reusing the existing ten states. **No decision is taken here and none is implied by this acknowledgement** — the Chief Editor decides the window, and `0002` stays unauthorized. **This is `Phase: 1` correctly**: the correction lands in Lane A's governing tiers.
- **Resolution:**
- **Verified-By:**
- **Evidence:**
- **Verified-At-Commit:**
