...
**Status:** Draft. **Specification only — nothing installed, no package added, no file created.**
...
**Limitations.** **bun is not installed on the Chief Editor's machine.** `bun test` therefore cannot be verified locally, and **CI is its first real execution.** This is the same shape as S1's `DEP-05` caveat and must be reported the same way: *"written, locally unverified"* — never *"done."*
...
## 5. `G59` — no lockfile is committed `[V1]` *(new)*

**Verified absent:** no `bun.lockb`, `package-lock.json`, `yarn.lock`, or `pnpm-lock.yaml` in the repository.

`bun install` in CI therefore **resolves dependencies fresh on every run.** Two consequences:
...
**Drafted fix:** commit `bun.lockb`, generated by `bun@1.1.30` to match the declared `packageManager`. **Cannot be produced on the Chief Editor's machine** — bun is not installed. Options: generate it in CI on first run and commit the artifact, or generate it on a machine with bun present.

**Severity:** T1, with `R3`. **Recorded, not resolved** — it needs an environment `R3`'s specification cannot supply.
...
|---|---|---|
| Standing build guardrail | **Blocking installation** | Specification permitted; file creation is not |
| bun on a developer machine | Blocks local verification and `G59` | Absent |
...
| No lockfile | Non-reproducible verdicts | `G59` |
...
**Creates no file, installs no package, adds no script.** Authorizes no code, schema, or migration. Does not resolve `TC6` — it compensates for it in CI. `G59` is recorded, not resolved. Installation remains gated on the standing build guardrail being lifted for `R3`.
